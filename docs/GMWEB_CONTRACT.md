...
`shared/eve-gmweb-contract-v1.json`. `panel/services/gmweb_contract.py` is the
only code that reads it, so endpoint paths, URL rules and request headers cannot
drift from the declaration: a mismatch fails
`tests/test_gmweb_contract.py` instead of failing a send in production.
...
`Accept: application/json`; `/send` also sends `Content-Type: application/json`
and, when the caller has one, `Idempotency-Key`. Request bodies are
`{"to", "text", "priority"}` with the priority lane mapped from Eve's canonical
kinds (critical 1, expired 3, expiring 6, announcement 10).
...
`tests/test_gmweb_contract.py` (19 tests) runs the real client functions against
a local fake gateway and asserts the wire contract:

* the contract file declares version 1, the four scopes and the five endpoints,
