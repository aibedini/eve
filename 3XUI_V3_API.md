...
- **Bearer token:** از Settings → Security → API Token. از **3.7.0** به بعد توکن‌ها **scope** دارند و می‌توانند **expire** شوند — ادعای قبلی («هرگز expire نمی‌شود») دیگر درست نیست. جزئیات در بخش «سازگاری نسخه‌ها» پایین.
- **Settings path:** در v3.3.1 زیر `/panel/api/setting/*` است (نه `/panel/setting/*`).

---

## سازگاری نسخه‌ها (v3.7.x / v3.8.x)

> تأییدشده روی تگ‌های دقیق `v3.7.0` = `f727d04f` و `v3.8.0` = `837addf6` (سورس + OpenAPI تولیدشده همان تگ)، و روی یک پنل واقعی 3.8.0. پذیرش پنل واقعی 3.7 اجرا نشده و با تصمیم محصول برای این انتشار معاف شده است؛ سازگاری هستهٔ 3.7 با تست‌های خودکار/قراردادی تأیید می‌شود.

EVE رفتار مخصوص هر نسخه را از یک لایهٔ مرکزی (`panel/services/xui_compat.py`) می‌گیرد. نسخهٔ پنل به عدد نرمال می‌شود، به خانوادهٔ `(major, minor)` کاهش می‌یابد، و فقط خانواده‌های صریحاً تأییدشده پروفایل غیرپایه می‌گیرند.

| خانوادهٔ نسخه | پشتیبانی CORE | پروفایل | یادداشت |
|---|---|---|---|
| legacy / pre-v3 | وضع موجود | legacy | بدون تغییر |
| 3.3.x – 3.6.x | وضع موجود | baseline_v3 | رفتار قبلی، بدون تغییر |
| **3.7.x** | **certified** | **xui_3_7** | توکن scope‌دار/expiring، `limitHwid`، اتوماسیون چرخهٔ عمر، AmneziaWG |
| **3.8.x** | **certified** | **xui_3_8** | همهٔ موارد 3.7 به‌علاوهٔ: 401 روی Bearer، مسیر ساب رندوم، TUIC |
| 3.9.x و 4.x | **unverified** | baseline_v3 | هشدار `future_version_uncertified` — **هرگز** پروفایل 3.8 را ارث نمی‌برد |
| نامشخص / غیرقابل‌تجزیه | **unverified** | baseline_v3 | هشدار `panel_version_unknown` — حدس زده نمی‌شود |

### تشخیص نسخه

- **منبع اصلی (محلی، بدون نیاز به اینترنت پنل):** `GET /panel/api/server/status` → `obj.panelVersion`. در هر دو تگ و به‌صورت پیوسته از v3.3.1 وجود دارد.
- **منبع تأییدکننده:** `GET /panel/api/server/getPanelUpdateInfo` → `obj.currentVersion`. این endpoint **اول به GitHub وصل می‌شود** و روی پنل بدون اینترنت `{"success":false}` **بدون `obj`** برمی‌گرداند. پس شکست آن به‌تنهایی هرگز نسخه را خراب نمی‌کند.
- نسخه هرگز به‌صورت رشته مقایسه نمی‌شود؛ `v3.8.0` ≡ `3.8.0` ≡ `3.8`.

### قواعد احراز هویت (3.7+)

| وضعیت | معنا |
|---|---|
| `2xx` | موفق |
| `401` | توکن نامعتبر / منقضی / چرخش‌خورده |
| `403` | احراز هویت شده ولی **scope کافی نیست** |
| `404` | مسیر/قابلیت وجود ندارد — فقط وقتی شواهد پشتیبانی کند |

- **Scope لازم برای مدیریت کامل EVE: `admin`.** توکن‌های `monitor` و `node-sync` برای مدیریت کامل کافی **نیستند** (allowlist آن‌ها `/clients/list`، `/clients/get/*`، `/setting/all`، `/clients/bulkEnable` و… را پوشش نمی‌دهد و `403` می‌دهد). EVE هرگز scope را خودسرانه بالا نمی‌برد؛ فقط خطای قابل‌اقدام نشان می‌دهد.
- **تفاوت مهم 3.7 و 3.8:** در 3.8 توکن Bearer نامعتبر **`401`** می‌گیرد؛ در 3.7 فقط اگر هدر `X-Requested-With: XMLHttpRequest` بفرستی `401` می‌گیری وگرنه **`404`**. EVE روی پروفایل 3.7 آن هدر را می‌فرستد تا «توکن خراب» از «مسیر ناموجود» قابل‌تفکیک باشد. `401` و `403` هرگز به‌معنای «پنل قدیمی» تفسیر و cache نمی‌شوند.
- **چرخش توکن:** `x-ui setting -getApiToken` یک توکن واحد (`cli-fallback`) را **rotate می‌کند** و توکن‌های قبلی را باطل می‌کند. توکنی که قبلاً چاپ شده از کار می‌افتد.

### `limitHwid` — چرا EVE مقدار را برمی‌گرداند

در `POST /panel/api/clients/update/{email}` فیلد `limitHwid` **خواهرِ** آبجکت client است (`model.Client` چنین فیلدی ندارد) و سرور آن را **بی‌قیدوشرط** می‌نویسد؛ کلید غایب ⇒ مقدار صفر ⇒ محدودیت دستگاه **پاک می‌شود**. این روی پنل واقعی 3.8.0 بازتولید شد (`2 → 0`).

مسیر خواندن EVE (`/inbounds/list` → `settings.clients[]`) این فیلد را **ندارد**؛ پس EVE مقدار معتبر را از `GET /panel/api/clients/get/{email}` → `obj.client.limitHwid` می‌خواند و در هر به‌روزرسانی بی‌ربط برمی‌گرداند. مقدار `0` یک انتخاب واقعی اپراتور است و همان‌طور برمی‌گردد. اگر خواندن معتبر شکست بخورد، عملیات **رد می‌شود** تا مقدار پاک نشود.

### اتوماسیون چرخهٔ عمر در پنل

3.7 فیلدهای `resetDay`، `resetMax`، `trafficReset` و `trafficResetDay` را اضافه کرد که به پنل اجازه می‌دهد خارج از دفترچهٔ EVE تمدید/ریست کند. EVE این وضعیت را **تشخیص و گزارش** می‌کند (`panel_lifecycle_automation_detected`، سرویس `partially_managed`) و هرگز آن فیلدها را نمی‌نویسد یا صفر نمی‌کند. تبدیل این رویدادها به رویدادهای چرخهٔ عمر EVE جزو این تغییر **نیست**.

### مسیر اشتراک در 3.8

نصب تازهٔ 3.8 مسیر اشتراک را **تصادفی** می‌سازد (`"/" + random.NumLower(16) + "/"`) در حالی که 3.7 مسیر ثابت `/sub/` داشت. EVE روی پروفایل 3.8 مسیر را از تنظیمات معتبر پنل می‌خواند و در نبود آن به مقدار پیکربندی‌شده برمی‌گردد و منبع را به‌عنوان fallback علامت می‌زند.
